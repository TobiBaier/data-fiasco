...
class FileManager:
    ...
    def load_setup(self):
        ...
        for i, identifier in enumerate(self.presets["identifiers"]):
            ...
            # dict is extracted from a json file -> keys should be allowed identifiers
                ...
                with open(self.path + self.presets["identifiers"][identifier]["path"], "r") as of:
    ...
    def get_identifiers(self, filename, return_id_names=False):

        filename = remove_path_syntax(filename)

        check_ret = self.check_identifiers(filename)
        name_elements = filename.split("_")
        id_names = name_elements.copy()
        ...
        if not check_ret:
            ...
            if return_id_names:
                return [i for i in name_elements if i is not False], [i for i in id_names if i is not False]
            else:
        ...
        # if all identifiers are at the specified locations, return them
            ...
            if return_id_names:
                return name_elements[0:len(self.identifiers)], list(self.identifiers.keys())
            else:
